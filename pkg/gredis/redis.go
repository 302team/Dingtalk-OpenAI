...
func Setup() error {
	RedisConn = &redis.Pool{
		MaxIdle:     public.Config.RedisMaxIdle,
		MaxActive:   public.Config.RedisMaxActive,
		IdleTimeout: time.Duration(public.Config.RedisIdleTime) * time.Second,
		Wait:        true,
		Dial: func() (redis.Conn, error) {
			c, err := redis.Dial("tcp", fmt.Sprintf("%s:%d", public.Config.RedisHost, public.Config.RedisPort), redis.DialPassword(public.Config.RedisPassword))
			if err != nil {
				return nil, err
			}
			return c, err
